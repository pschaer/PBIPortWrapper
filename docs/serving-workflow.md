...
[docs/research/](research/README.md) for the investigation that led to this decision.
...
- **PBIRelay crash**: on next start, a recovery record + a live instance whose DB
...
- **Multi-user LAN sharing** needs the **auth-terminating HTTP/XMLA bridge**
  ...
  leg only. Research links in [docs/research/](research/README.md).
...
| E5 | Desktop auto-recovery behavior after a session that ended while renamed | Recovery UX | **Done — clean close, clean reopen (fresh workspace: new port, new GUID), zero residue. Desktop needs no recovery from us; the recovery record covers only the PBIRelay-crash-while-serving case, matched by immutable DB ID (#43)** |

## Release ladder (always shippable)

- **v0.4** — core extraction: headless `PBIRelay.Core` (state out of the grid),
...
framing + SSPI sealing over the MS-SSAS transport, with content negotiation, MS-BINXML
and Xpress9). Revisit only if a use case appears that renaming cannot serve.
