...
namespace PBIPortWrapper.Services
{
    public class ProxyManager
    {
        ...
        /// <summary>
        /// The backend port a running proxy currently forwards to, or null if no
        /// proxy runs on the given fixed port. Used to detect stale targets after
        /// Desktop restarts (#49).
        /// </summary>
        public int? GetTargetPort(int fixedPort)
        {
            if (_proxies.TryGetValue(fixedPort, out var proxy) && proxy.IsRunning)
            {
        ...
        public int GetAllActiveConnectionsCount()
        {
            return _proxies.Values.Sum(p => p.ActiveConnections);
        }
        ...
        public IReadOnlyList<ConnectionInfo> GetConnectionDetails(int fixedPort)
        {
            if (_proxies.TryGetValue(fixedPort, out var proxy))
            {
                return proxy.ActiveConnectionDetails;
            }
